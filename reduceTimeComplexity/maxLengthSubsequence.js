...
function solution(nums, k){
  ...
  for(let rt = 0; rt < nums.length; rt += 1) {
    ...
    // 그 카운팅이 0을 1로 바꿀 수 있는 횟수를 초과하게 되면 다음 while문이 수행된다.
    while(cnt > k) {
      // 왼쪽 요소가 0이면 cnt를 줄이고 왼쪽 위치를 오른쪽으로 하나 옮기고 다시 for문을 돈다
      // 왼쪽 요소가 1이면 그냥 왼쪽 위치를 오른쪽으로 하나 옮긴다. 
      // 그리고 여전히 cnt가 k보다 크므로 옮긴 왼쪽이 0인지 1인지 확인하며 if문 수행여부를 결정한다.
      if(nums[lt] === 0) cnt -= 1
      lt += 1
